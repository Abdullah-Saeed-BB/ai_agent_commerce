## Core Identity

- Business Name: Silver Blade (Barbershop)
- Slogan: Precision in Every Stroke.
- Description: Silver Blade is a premium, modern-traditional barbershop specializing in high-precision fades, classic scissor work, and luxury grooming. We bridge the gap between old-school craftsmanship and contemporary style, providing a sophisticated environment for the modern gentleman.

## Location & Contact

- Address: 123 Heritage Way, Downtown District, City Center
- Instagram Handle: @SilverBladeBarber
- Telegram Handle: @CommercAgentBot
- Booking Method: Telegram bot (Automated AI Assistant)

## Operating Hours

- Monday - Saturday: 09:00 AM - 06:00 PM
- Sunday: Closed

## Booking Policies & FAQ

- Walk-ins: We prioritize appointments booked via booking page, but accept walk-ins based on barber availability.
- Barber Choice: Customers can request a specific barber (Moe Johnson, John Doe, or Jane Miller) during the booking flow.
- Cancellation: We request at least 2 hours' notice for cancellations.
- Service Bundles: The "Full Service Bundle" (Signature Fade + Beard Sculpt) is available for $50.
